...
Процедура ВыполнитьКомандуЗапросаСоставаКМ(Параметры)
	...
	РезультатОбработки = ЗапросСоставаКМОбработатьШтрихкоды(Параметры.Штрихкоды, ОтключитьЗапросКСервису);
	...
	Если РезультатОбработки.ШтрихкодыКОтправке.Количество() > 0 Тогда
		...
		Если ОтключитьЗапросКСервису Тогда
			ЗапросСоставаКМЗапроситьИзИБ(РезультатОбработки.ШтрихкодыКОтправке, ШтрихкодыРезультат);
		Иначе
		...
		Для Каждого СоответствиеШтрихкодовКЗ Из РезультатОбработки.СоответствияШтрихкодов Цикл
			...
			Если РезультатШК = Неопределено И РезультатШКОшибка = Неопределено Тогда
				ТекстОшибки = НСтр("ru = 'Не удалось получить результат запроса по штрихкоду.'");
...
Функция ЗапросСоставаКМОбработатьШтрихкоды(Штрихкоды, НачинаетсяСоСкобки = Ложь)
	...
	Если Штрихкоды.Количество() > 0 Тогда
		...
		ПараметрыНормализацииМОТП.НачинаетсяСоСкобки = НачинаетсяСоСкобки;
		...
		ПараметрыНормализацииИСМПУкороченныйФормат.НачинаетсяСоСкобки = НачинаетсяСоСкобки;
		
		ПараметрыНормализацииИСМПЛидирующиеНули = РазборКодаМаркировкиИССлужебныйКлиентСервер.ПараметрыНормализацииКодаМаркировки();
		ПараметрыНормализацииИСМПЛидирующиеНули.НачинаетсяСоСкобки = НачинаетсяСоСкобки;
...
Процедура ЗапросСоставаКМЗапроситьИзИБ(ШтрихкодыКЗапросу, ШтрихкодыРезультат)
	...
	Запрос.УстановитьПараметр("ШтрихкодыКЗапросу", ШтрихкодыКЗапросу);
	...
	Если Не РезультатЗапроса.Пустой() Тогда
		...
		РезультатВложенныеШК = ШтрихкодированиеИС.ВложенныеШтрихкодыУпаковок(
			РезультатЗапроса.Выгрузить().ВыгрузитьКолонку("Ссылка"));
		
		Для Каждого СтрокаДереваУпаковок Из РезультатВложенныеШК.ДеревоУпаковок.Строки Цикл
			
			ДанныеКМ = ЗапросСоставаКМСтруктураШтрихкод(СтрокаДереваУпаковок.НормализованныйШтрихкод, 
				СтрокаДереваУпаковок.Штрихкод);
			
			ЗапросСоставаКМИзИБДополнитьДочерними(СтрокаДереваУпаковок.Строки, ДанныеКМ.child);
			
			ШтрихкодыРезультат.Вставить(СтрокаДереваУпаковок.НормализованныйШтрихкод, ДанныеКМ);
...
Процедура ЗапросСоставаКМИзИБДополнитьДочерними(СтрокиДерева, МассивПриемник)
	
	Для Каждого СтрокаДерева Из СтрокиДерева Цикл
		
		ДанныеКМ = ЗапросСоставаКМСтруктураШтрихкод(СтрокаДерева.НормализованныйШтрихкод, СтрокаДерева.Штрихкод);
		
		ЗапросСоставаКМИзИБДополнитьДочерними(СтрокаДерева.Строки, ДанныеКМ.child);
		
		МассивПриемник.Добавить(ДанныеКМ);
