...
Процедура ВыполнитьКоманду(ИдентификаторКоманды, ПараметрыВыполнения) Экспорт
	...
	Попытка
		...
		ИначеЕсли ИдентификаторКоманды = "ЗапросСоставаКодовМаркировки" Тогда
			ЗапросСостваКМ_Выполнить(ПараметрыВыполнения);
...
#Область НастройкиИнтеграционнойОбработки

Функция НовыеНастройкиЭталон() Экспорт
	
	НовыеНастройки = Новый Структура();
	
	НовыеНастройки.Вставить("ЗапросСоставаКМ", Новый Структура());
	НовыеНастройки.ЗапросСоставаКМ.Вставить("ОтключитьЗапросКСервису", Ложь);
...
Функция ПолучитьНастройки() Экспорт
	...
	НастройкиИзХранилища = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить(
		КлючОбъектаНастроек(), КлючОбщихНастроек(), , , КлючПользователяНастроек());
		
	УстановитьПривилегированныйРежим(Ложь);
	
	Возврат НастройкиИзХранилища;
...
Процедура СохранитьНастройки(Настройки) Экспорт
	...
	ОбщегоНазначения.ХранилищеОбщихНастроекСохранить(
		КлючОбъектаНастроек(), КлючОбщихНастроек(), Настройки, , КлючПользователяНастроек());
...
Процедура ДобавитьКомандыОбработки(ПараметрыРегистрации)
	...
	// Команда ОткрытиеФормыНастройки
	НоваяКоманда = ПараметрыРегистрации.Команды.Добавить();
	НоваяКоманда.Представление = НСтр("ru = 'Настройки'");
	НоваяКоманда.Идентификатор = "ОткрытиеФормыНастройки";
...
Функция КлючОбъектаНастроек()
	...
	Возврат "SimpleИнтеграцияГИСМТОбработка";
...
Функция КлючПользователяНастроек()
	
	Возврат "SimpleИнтеграцияГИСМТОбщийПользователь";
...
Функция РазобратьКодМаркировки(Знач Штрихкод, НастройкиРазбора, ВидыПродукции)
	...
	Иначе
		Результат.Ошибка = НСтр("ru = 'Не удалось получить результат разбора кода маркировки!'");
	КонецЕсли;
...
#Область КомандыОбработки

Процедура ПроверитьПараметрыКомандыПриНачалеВыполнения(Параметры, ПроцедураВызова, Знач ОжидаемыеТипыСвойств = Неопределено)
	
	Если ТипЗнч(ОжидаемыеТипыСвойств) <> Тип("Структура") Тогда
		ОжидаемыеТипыСвойств = Новый Структура();
	КонецЕсли;
	
	ОжидаемыеТипыСвойств.Вставить("РезультатВыполнения", Тип("Структура"));
...
Функция ПолучитьПараметрыСканированияЗапросаСоставаКМПоУпаковкам()
	...
	ПараметрыСканирования.Вставить("ПодсистемаИСМПСуществует", ОбщегоНазначения.ПодсистемаСуществует("ГосИС.ИСМП"));
	...
	ПараметрыСканирования.Вставить("РазрешенаОбработкаНеНайденныхЛогистическихУпаковок", Истина);
	ПараметрыСканирования.Вставить("Владелец", 										Неопределено);
	ПараметрыСканирования.Вставить("ИмяТабличнойЧастиШтрихкодыУпаковок", 			Неопределено);
	ПараметрыСканирования.Вставить("ВозможнаЗагрузкаТСД", 							Истина);
	ПараметрыСканирования.Вставить("РазрешенаОбработкаКодовСПустойНоменклатурой", 	Истина);
	...
	ПараметрыСканирования.Вставить("ТолькоМаркируемаяПродукция", 					Истина);
	ПараметрыСканирования.Вставить("КонтрольУникальностиКодовМаркировки", 			Истина);
	ПараметрыСканирования.Вставить("ДопустимыеВидыПродукции", ОбщегоНазначенияИСКлиентСервер.ВидыПродукцииИСМП(Истина));
...
//	ПараметрыСканирования.Вставить("ЗаполнятьСрокГодности", Истина);
...
Процедура ВыполнитьКомандуПолучитьВложенныеШтрихкодыУпаковок(Параметры)
	...
	Если ШтрихкодированиеОбщегоНазначенияИС.ДопустимаТабачнаяПродукция(ПараметрыСканирования)
		...
		МодульШтрихкодированиеИСМП.ДобавитьКолонкиТаблицыДанныеКодовМаркировки(ДанныеПоШтрихкодам, ПараметрыСканирования, ДанныеПоШтрихкодам.ДанныеКодовМаркировки);
	...
	ЗаполнитьМассивВложенныхШтрихкодовПоСтрокамДерева(ВложенныеШтрихкодыСостав, ВложенныеШтрихкоды.ДеревоУпаковок.Строки, МассивШтрихкодовУпаковок);
	...
	Параметры.РезультатВыполнения.Вставить("МассивШтрихкодовУпаковок", МассивШтрихкодовУпаковок);
...
Процедура ЗаполнитьМассивВложенныхШтрихкодовПоСтрокамДерева(МассивВложенныхШтрихкодов, СтрокиДерева, МассивШтрихкодовУпаковок)
	...
	Для Каждого СтрокаДерева Из СтрокиДерева Цикл
		...
		Если ЗначениеЗаполнено(СтрокаДерева.ШтрихкодУпаковки) И 
			МассивШтрихкодовУпаковок.Найти(СтрокаДерева.ШтрихкодУпаковки) = Неопределено Тогда
			МассивШтрихкодовУпаковок.Добавить(СтрокаДерева.ШтрихкодУпаковки);
		КонецЕсли;
		
		ЗаполнитьМассивВложенныхШтрихкодовПоСтрокамДерева(ДанныеСтроки.ВложенныеШтрихкоды, СтрокаДерева.Строки, МассивШтрихкодовУпаковок);
...
Процедура ЗапросСостваКМ_Выполнить(Параметры)
	
	ОжидаемыеТипыСвойств = Новый Структура();
	ОжидаемыеТипыСвойств.Вставить("Штрихкоды", Тип("Массив"));
	
	ПроверитьПараметрыКомандыПриНачалеВыполнения(Параметры, "ЗапросСостваКМ_Выполнить", ОжидаемыеТипыСвойств);
	
	ИспользуемыеМодули = ЗапросСостваКМ_ИспользуемыеМодули();
	
	ДанныеШтрихкодовКОбработке = ЗапросСоставаКМ_ШтрихкодыКОбработке(Параметры.Штрихкоды, ИспользуемыеМодули);
	
	НастройкиОбработки = ПолучитьНастройки();
	ОтключитьЗапросКСервису = СвойствоСтруктуры(НастройкиОбработки, "ЗапросСоставаКМ.ОтключитьЗапросКСервису", Ложь);
	
	ПараметрыСканирования = ЗапросСоставаКМ_ПолучитьПараметрыСканирования(ИспользуемыеМодули);
	
	Если ОтключитьЗапросКСервису Тогда
	...
	РезультатОбработкиШтрихкодов = ИспользуемыеМодули.ШтрихкодированиеОбщегоНазначенияИС.ОбработатьШтрихкоды(
		ДанныеШтрихкодовКОбработке.ШтрихкодыСписок, ПараметрыСканирования, Неопределено, ИдентификаторОперации);
	
	Если Не ОтключитьЗапросКСервису И РезультатОбработкиШтрихкодов.ДанныеПоШтрихкодам.ТребуетсяОбновлениеКлючаСессии Тогда
		
		ЗапросСоставаКМ_ПолучитьКлючСессии(ИспользуемыеМодули);
	...
	ЗапросСоставаКМ_ЗаполнитьДанныеПоШтрихкодамУпаковокИБ(ИспользуемыеМодули,
		РезультатОбработкиШтрихкодов.ДанныеПоШтрихкодам.ДанныеКодовМаркировки);
	
	Если ОтключитьЗапросКСервису Тогда
		
		ЗапросСоставаКМ_ЗаполнитьДанныеПоВложеннымШтрихкодамУпаковокИБ(ИспользуемыеМодули,
			РезультатОбработкиШтрихкодов.ДанныеПоШтрихкодам, ПараметрыСканирования);
		
	КонецЕсли;
	
	РезультатСоставКодовМаркировки = ЗапросСоставаКМ_СформироватьРезультат(ИспользуемыеМодули, 
		ДанныеШтрихкодовКОбработке.ТаблицаШтрихкоды,
		РезультатОбработкиШтрихкодов.ДанныеПоШтрихкодам.ДанныеКодовМаркировки);
	
	Параметры.РезультатВыполнения.Вставить("СоставКодовМаркировки", РезультатСоставКодовМаркировки);
	
КонецПроцедуры

Функция ЗапросСостваКМ_ИспользуемыеМодули()
	
	ИспользуемыеМодули = Новый Структура();
	
	ИспользуемыеМодули.Вставить("ШтрихкодированиеОбщегоНазначенияИСКлиентСервер", 
		ОбщегоНазначения.ОбщийМодуль("ШтрихкодированиеОбщегоНазначенияИСКлиентСервер"));
	ИспользуемыеМодули.Вставить("ОбщегоНазначенияИСКлиентСервер", ОбщегоНазначения.ОбщийМодуль("ОбщегоНазначенияИСКлиентСервер"));
	ИспользуемыеМодули.Вставить("ШтрихкодированиеИСКлиентСервер", ОбщегоНазначения.ОбщийМодуль("ШтрихкодированиеИСКлиентСервер"));
	ИспользуемыеМодули.Вставить("ШтрихкодированиеОбщегоНазначенияМОТПКлиентСервер",
		ОбщегоНазначения.ОбщийМодуль("ШтрихкодированиеОбщегоНазначенияМОТПКлиентСервер"));
	ИспользуемыеМодули.Вставить("ОбщегоНазначенияИСМПКлиентСервер", ОбщегоНазначения.ОбщийМодуль("ОбщегоНазначенияИСМПКлиентСервер"));
	ИспользуемыеМодули.Вставить("ШтрихкодированиеИСМПКлиентСервер", ОбщегоНазначения.ОбщийМодуль("ШтрихкодированиеИСМПКлиентСервер"));
	ИспользуемыеМодули.Вставить("ИнтерфейсИСМПОбщегоНазначенияКлиентСервер", ОбщегоНазначения.ОбщийМодуль("ИнтерфейсИСМПОбщегоНазначенияКлиентСервер"));
	ИспользуемыеМодули.Вставить("ИнтерфейсАвторизацииИСМПСлужебный", ОбщегоНазначения.ОбщийМодуль("ИнтерфейсАвторизацииИСМПСлужебный"));
	ИспользуемыеМодули.Вставить("ШтрихкодированиеОбщегоНазначенияИС", ОбщегоНазначения.ОбщийМодуль("ШтрихкодированиеОбщегоНазначенияИС"));
	ИспользуемыеМодули.Вставить("ШтрихкодированиеИС", ОбщегоНазначения.ОбщийМодуль("ШтрихкодированиеИС"));
	ИспользуемыеМодули.Вставить("ШтрихкодированиеИСМПСлужебный", ОбщегоНазначения.ОбщийМодуль("ШтрихкодированиеИСМПСлужебный"));
	ИспользуемыеМодули.Вставить("ИнтерфейсИСМПОбщегоНазначения", ОбщегоНазначения.ОбщийМодуль("ИнтерфейсИСМПОбщегоНазначения"));
	
	Возврат ИспользуемыеМодули;
	
КонецФункции

Функция ЗапросСоставаКМ_ШтрихкодыКОбработке(Штрихкоды, ИспользуемыеМодули)
	
	ТаблицаШтрихкоды = Новый ТаблицаЗначений();
	ТаблицаШтрихкоды.Колонки.Добавить("Штрихкод", ОбщегоНазначения.ОписаниеТипаСтрока(200));
	ТаблицаШтрихкоды.Колонки.Добавить("ШтрихкодBase64", ОбщегоНазначения.ОписаниеТипаСтрока(500));
	
	Результат = Новый Структура();
	Результат.Вставить("ТаблицаШтрихкоды", ТаблицаШтрихкоды);
	Результат.Вставить("ШтрихкодыСписок", Новый Массив());
	
	
	Для индекс = 0 По Штрихкоды.ВГраница() Цикл
		
		Если ТаблицаШтрихкоды.Найти(Штрихкоды[индекс], "Штрихкод") <> Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		ДанныеШтрихкода = Новый Структура();
		ДанныеШтрихкода.Вставить("Количество", 1);
		ДанныеШтрихкода.Вставить("Штрихкод", Штрихкоды[индекс]);
		
		ИспользуемыеМодули.ШтрихкодированиеОбщегоНазначенияИСКлиентСервер.ЗакодироватьШтрихкодДанныхBase64(ДанныеШтрихкода);
		ИспользуемыеМодули.ШтрихкодированиеОбщегоНазначенияИСКлиентСервер.ДекодироватьШтрихкодДанныхBase64(ДанныеШтрихкода, Истина);
		
		Результат.ШтрихкодыСписок.Добавить(ДанныеШтрихкода);
		
		СтрокаТаблицыШтрихкодов = ТаблицаШтрихкоды.Добавить();
		СтрокаТаблицыШтрихкодов.Штрихкод = ДанныеШтрихкода.Штрихкод;
		СтрокаТаблицыШтрихкодов.ШтрихкодBase64 = ДанныеШтрихкода.ШтрихкодBase64;
...
Функция ЗапросСоставаКМ_ПолучитьПараметрыСканирования(ИспользуемыеМодули)
	
	ПараметрыСканирования = ИспользуемыеМодули.ШтрихкодированиеОбщегоНазначенияИСКлиентСервер.БазовыеПараметрыСканирования();
	
	ПараметрыСканирования.Вставить("ПодсистемаИСМПСуществует", ОбщегоНазначения.ПодсистемаСуществует("ГосИС.ИСМП"));
	ПараметрыСканирования.Вставить("ИдентификаторФормы");
	ПараметрыСканирования.Вставить("ДопустимыеВидыПродукции", Новый Массив());
	ПараметрыСканирования.Вставить("КэшМаркируемойПродукции", "");
	
	Если ИспользуемыеМодули.ОбщегоНазначенияИСКлиентСервер.ЭтоРасширеннаяВерсияГосИС("ИСМП") Тогда
		
		ЗапросСоставаКМ_ЗаполнитьПараметрыСканированияДляТабачнойПродукции(ПараметрыСканирования, ИспользуемыеМодули);
		
		ЗапросСоставаКМ_ЗаполнитьПараметрыСканированияДляПродукцииИСМП(ПараметрыСканирования, ИспользуемыеМодули);
		
		ИспользуемыеМодули.ШтрихкодированиеИСКлиентСервер.ЗаполнитьПараметрыСканирования(ПараметрыСканирования, Неопределено);
		
	КонецЕсли;
	
	ПараметрыСканирования.Вставить("Организация", 	Неопределено);
	ПараметрыСканирования.Вставить("Владелец", 		Неопределено);
	ПараметрыСканирования.Вставить("РабочееМесто", 	ИспользуемыеМодули.ОбщегоНазначенияИСКлиентСервер.РабочееМестоПользователя());
	ПараметрыСканирования.Вставить("КонтрольУникальностиКодовМаркировки", Ложь);
	ПараметрыСканирования.Вставить("ВыводитьСообщенияОбОшибках", Ложь);
	ПараметрыСканирования.Вставить("ЗаписыватьЛогЗапросовИСМП", Ложь);
	ПараметрыСканирования.Вставить("ДопустимыеВидыПродукции", ИспользуемыеМодули.ОбщегоНазначенияИСКлиентСервер.ВидыПродукцииИСМП(Истина));
	
	Возврат ПараметрыСканирования;
	
КонецФункции

Процедура ЗапросСоставаКМ_ЗаполнитьПараметрыСканированияДляТабачнойПродукции(ПараметрыСканирования, ИспользуемыеМодули)
	
	Если Не ИспользуемыеМодули.ШтрихкодированиеОбщегоНазначенияМОТПКлиентСервер.ВключитьПоддержкуТабачнойПродукции(ПараметрыСканирования,, Ложь) Тогда
		Возврат;
	КонецЕсли;
	...
	ПараметрыСканирования.Вставить("ПроверятьПотребительскиеУпаковкиНаВхождениеВСеруюЗонуМОТП", 			Истина);
	ПараметрыСканирования.Вставить("ПроверятьЛогистическиеИГрупповыеУпаковкиНаСодержаниеСерыхКодовМОТП", 	Истина);
	ПараметрыСканирования.Вставить("ЗаписыватьЛогЗапросовИСМП",  											Истина);
	ПараметрыСканирования.Вставить("ТребуетсяЧастичноеВыбытие", 											Истина);
	ПараметрыСканирования.Вставить("СозданиеНаборов", 														Истина);
	
	ИспользуемыеМодули.ОбщегоНазначенияИСМПКлиентСервер.УстановитьПараметрСканированияЗапрашиватьДанныеНеизвестныхУпаковокИСМП(ПараметрыСканирования, Истина);
	ИспользуемыеМодули.ОбщегоНазначенияИСМПКлиентСервер.ЗначениеПараметраСканированияЗапрашиватьДанныеСервисаИСМП(ПараметрыСканирования, Истина);
	
КонецПроцедуры

Процедура ЗапросСоставаКМ_ЗаполнитьПараметрыСканированияДляПродукцииИСМП(ПараметрыСканирования, ИспользуемыеМодули)
	
	Если Не ИспользуемыеМодули.ШтрихкодированиеИСМПКлиентСервер.ВключитьПоддержкуПродукцииИСМП(ПараметрыСканирования,, Ложь) Тогда
		Возврат;
	КонецЕсли;
	...
	ИспользуемыеМодули.ОбщегоНазначенияИСМПКлиентСервер.УстановитьПараметрСканированияЗапрашиватьДанныеНеизвестныхУпаковокИСМП(ПараметрыСканирования, Истина);
	ИспользуемыеМодули.ОбщегоНазначенияИСМПКлиентСервер.УстановитьПараметрСканированияЗапрашиватьДанныеСервисаИСМП(ПараметрыСканирования, Истина);
	
КонецПроцедуры

Функция ЗапросСоставаКМ_ПолучитьКлючСессии(ИспользуемыеМодули, Организация = Неопределено)
	
	ПараметрыЗапроса = ИспользуемыеМодули.ИнтерфейсИСМПОбщегоНазначенияКлиентСервер.ПараметрыЗапросаКлючаСессии();
	ПараметрыЗапроса.Вставить("ПредставлениеСервиса", НСтр("ru = 'Simple: Интеграция ГИС МТ'"));
	ПараметрыЗапроса.Вставить("Организация", Организация);
	
	КлючСесии = ИспользуемыеМодули.ИнтерфейсАвторизацииИСМПСлужебный.ПроверитьОбновитьКлючСессии(ПараметрыЗапроса, Неопределено, Истина);
	
	ОбщегоНазначенияКлиентСервер.Проверить(
		Не ПустаяСтрока(ОбщегоНазначения.ОписаниеТипаСтрока(0).ПривестиЗначение(КлючСесии)), 
		НСтр("ru = 'Не удалось получить ключ сессии'"));
	
	Возврат КлючСесии;
	
КонецФункции

Процедура ЗапросСоставаКМ_ЗаполнитьДанныеПоШтрихкодамУпаковокИБ(ИспользуемыеМодули, ДанныеКодовМаркировки)
	
	ШтрихкодыУпаковокИБ = ИспользуемыеМодули.ШтрихкодированиеИС.ШтрихкодыУпаковок(
		ДанныеКодовМаркировки.ВыгрузитьКолонку("Штрихкод"));
	
	Для Каждого СтрокаШтрихкодыУпаковокИБ Из ШтрихкодыУпаковокИБ Цикл
		
		ПараметрыОтбораСтрок = Новый Структура();
		ПараметрыОтбораСтрок.Вставить("Штрихкод", СтрокаШтрихкодыУпаковокИБ.Штрихкод);
		НайденныеСтрокиКМ = ДанныеКодовМаркировки.НайтиСтроки(ПараметрыОтбораСтрок);
		
		Для индекс = 0 По НайденныеСтрокиКМ.ВГраница() Цикл
			
			Если Не ЗначениеЗаполнено(НайденныеСтрокиКМ[индекс].ШтрихкодУпаковки) Тогда
				НайденныеСтрокиКМ[индекс].ШтрихкодУпаковки = СтрокаШтрихкодыУпаковокИБ.Ссылка;
			КонецЕсли;
			
			Если Не ЗначениеЗаполнено(НайденныеСтрокиКМ[индекс].Номенклатура) Тогда
				НайденныеСтрокиКМ[индекс].Номенклатура = СтрокаШтрихкодыУпаковокИБ.Номенклатура;
			КонецЕсли;
			
			Если Не ЗначениеЗаполнено(НайденныеСтрокиКМ[индекс].Характеристика) Тогда
				НайденныеСтрокиКМ[индекс].Характеристика = СтрокаШтрихкодыУпаковокИБ.Характеристика;
			КонецЕсли;
			
			Если Не ЗначениеЗаполнено(НайденныеСтрокиКМ[индекс].Серия) Тогда
				НайденныеСтрокиКМ[индекс].Серия = СтрокаШтрихкодыУпаковокИБ.Серия;
			КонецЕсли;
			
			Если Не ЗначениеЗаполнено(НайденныеСтрокиКМ[индекс].ТипУпаковки) Тогда
				НайденныеСтрокиКМ[индекс].ТипУпаковки = СтрокаШтрихкодыУпаковокИБ.ТипУпаковки;
			КонецЕсли;
...
Процедура ЗапросСоставаКМ_ЗаполнитьДанныеПоВложеннымШтрихкодамУпаковокИБ(ИспользуемыеМодули, ДанныеПоШтрихкодам, ПараметрыСканирования)
	
	ДанныеПоШтрихкодам.ВложенныеШтрихкоды = ИспользуемыеМодули.ШтрихкодированиеИС.ВложенныеШтрихкодыУпаковок(
		ДанныеПоШтрихкодам.ДанныеКодовМаркировки.ВыгрузитьКолонку("ШтрихкодУпаковки"));
	
	ИспользуемыеМодули.ШтрихкодированиеИСМПСлужебный.НормализоватьДанныеДереваУпаковок(
		ДанныеПоШтрихкодам.ВложенныеШтрихкоды.ДеревоУпаковок,
		ПараметрыСканирования,
		ДанныеПоШтрихкодам.ДанныеКодовМаркировки);
	
	Для Каждого СтрокаДанныхКМ Из ДанныеПоШтрихкодам.ДанныеКодовМаркировки Цикл
		
		Если Не ЗначениеЗаполнено(СтрокаДанныхКМ.ШтрихкодУпаковки) Тогда
			Продолжить;
		КонецЕсли;
		
		СтрокаДанныхКМ.СтрокаДерева = ДанныеПоШтрихкодам.ВложенныеШтрихкоды.ДеревоУпаковок.Строки.Найти(
			СтрокаДанныхКМ.ШтрихкодУпаковки, "ШтрихкодУпаковки", Истина);
		
	КонецЦикла;
	
КонецПроцедуры

Функция ЗапросСоставаКМ_СформироватьРезультат(ИспользуемыеМодули, ТаблицаШтрихкоды, ДанныеКодовМаркировки)
	
	Результат = Новый Массив();
	
	Для Каждого СтрокаТаблицыШтрихкоды Из ТаблицаШтрихкоды Цикл
		
		НайденнаяСтрока = ДанныеКодовМаркировки.Найти(СтрокаТаблицыШтрихкоды.ШтрихкодBase64, "ШтрихкодBase64");
		
		ДанныеШК = ЗапросСоставаКМ_ОбработатьСтрокуДанныхКодовМаркировки(
			СтрокаТаблицыШтрихкоды.Штрихкод, НайденнаяСтрока, ИспользуемыеМодули);
		
		Результат.Добавить(ДанныеШК);
...
Функция ЗапросСоставаКМ_ОбработатьСтрокуДанныхКодовМаркировки(ИсходныйШтрихкод, СтрокаДанныхКМ, ИспользуемыеМодули, ЭтоСтрокаДерева = Ложь)
	
	РезультатОбработки = Новый Структура();
	РезультатОбработки.Вставить("requestedCis", ИсходныйШтрихкод);
		
	Если СтрокаДанныхКМ = Неопределено Тогда
		ТекстОшибки = НСтр("ru = 'Данные не найдены!'");
	ИначеЕсли Не ПустаяСтрока(СтрокаДанныхКМ.ТекстОшибки) Тогда 
		ТекстОшибки = СтрокаДанныхКМ.ТекстОшибки;
	Иначе
		ТекстОшибки = "";
	КонецЕсли;
	
	Если Не ПустаяСтрока(ТекстОшибки) Тогда
		РезультатОбработки.Вставить("error", ТекстОшибки);
	Иначе
		
		РезультатОбработки.Вставить("cis", СтрокаДанныхКМ.Штрихкод);
		РезультатОбработки.Вставить("ownerInn", СтрокаДанныхКМ.ИННВладельца);
		РезультатОбработки.Вставить("status");
		
		ТипЗначенияСтатус = ТипЗнч(СтрокаДанныхКМ.Статус);
		
		Если ТипЗначенияСтатус = Тип("ПеречислениеСсылка.СтатусыКодовМаркировкиИСМП") Тогда
			РезультатОбработки.Вставить("status", 
				ИспользуемыеМодули.ИнтерфейсИСМПОбщегоНазначения.СтатусКодаМаркировкиИСМП(СтрокаДанныхКМ.Статус));
		ИначеЕсли ТипЗначенияСтатус = Тип("ПеречислениеСсылка.СтатусыКодовМаркировкиМОТП") Тогда
			РезультатОбработки.Вставить("status", 
				ИспользуемыеМодули.ИнтерфейсИСМПОбщегоНазначения.СтатусКодаМаркировкиМОТП(СтрокаДанныхКМ.Статус));
		ИначеЕсли СтрокаДанныхКМ.Статус <> Неопределено Тогда
			РезультатОбработки.Вставить("status", Строка(СтрокаДанныхКМ.Статус));
		Иначе
			РезультатОбработки.Вставить("status");
		КонецЕсли;
		
		Если СтрокаДанныхКМ.ВидУпаковки = Перечисления.ВидыУпаковокИС.Потребительская Тогда
			РезультатОбработки.Вставить("packType", "SINGLE");
		Иначе
			РезультатОбработки.Вставить("packType", "GROUP");
		КонецЕсли;
		
		РезультатОбработки.Вставить("child", Новый Массив());
		
		Если ЭтоСтрокаДерева Тогда
			СтрокаДерева = СтрокаДанныхКМ;
		Иначе
			СтрокаДерева = СтрокаДанныхКМ.СтрокаДерева;
		КонецЕсли;
		
		Если СтрокаДерева <> Неопределено Тогда
			
			Для Каждого ЭлементСтрока Из СтрокаДерева.Строки Цикл
				
				ДанныеШК = ЗапросСоставаКМ_ОбработатьСтрокуДанныхКодовМаркировки(ЭлементСтрока.Штрихкод, ЭлементСтрока, 
					ИспользуемыеМодули, Истина);
				
				РезультатОбработки.child.Добавить(ДанныеШК);
...
Процедура ВыполнитьКомандуЗапросаСоставаКМ(Параметры)
	...
	ДеревоШтрихкодов = ЗапросСоставаКМСформироватьДеревоЗапрашиваемыхШтрихкодов(Параметры.Штрихкоды);
	
	НастройкиОбработки = ПолучитьНастройки();
	ОтключитьЗапросКСервису = СвойствоСтруктуры(НастройкиОбработки, "ЗапросСоставаКМ.ОтключитьЗапросКСервису", Ложь);
	
	Если ОтключитьЗапросКСервису Тогда
		ДеревоЗапрошенныхШтрихкодов = ЗапросСоставаКМПолучитьШтрихкодыПоДаннымИБ(ДеревоШтрихкодов);
		ИмяКолонкиЗначения = "НормализованныйШтрихкод";
	Иначе
		ДеревоЗапрошенныхШтрихкодов = ЗапросСоставаКМНачатьПолучениеШтрихкодовПоДаннымСервиса(ДеревоШтрихкодов);
		ИмяКолонкиЗначения = "Штрихкод";
	КонецЕсли;
	
	Для Каждого СтрокаШтрихкода Из ДеревоШтрихкодов.Строки Цикл
		
		Если Не ПустаяСтрока(СтрокаШтрихкода.Ошибка) Тогда
			Продолжить;
		КонецЕсли;
		
		НайденнаяСтрокаЗапрошенногоШтрихкода = Неопределено;
		
		Для Каждого ПодчиненнаяСтрокаШтрихкода Из СтрокаШтрихкода.Строки Цикл
			
			НайденнаяСтрокаЗапрошенногоШтрихкода = ДеревоЗапрошенныхШтрихкодов.Строки.Найти(
				ПодчиненнаяСтрокаШтрихкода[ИмяКолонкиЗначения], "Штрихкод", Истина);
			
			Если НайденнаяСтрокаЗапрошенногоШтрихкода <> Неопределено Тогда
				Прервать;
			КонецЕсли;
			
		КонецЦикла;
		
		СтрокаШтрихкода.Строки.Очистить();
		
		Если НайденнаяСтрокаЗапрошенногоШтрихкода = Неопределено Тогда
			СтрокаШтрихкода.Ошибка = НСтр("ru = 'Данные по штрихкоду не получены!'");
		Иначе
			СтрокаШтрихкода.НормализованныйШтрихкод = НайденнаяСтрокаЗапрошенногоШтрихкода.НормализованныйШтрихкод;
			СтрокаШтрихкода.Ошибка = НайденнаяСтрокаЗапрошенногоШтрихкода.Ошибка;
			СтрокаШтрихкода.ИННВладельца = НайденнаяСтрокаЗапрошенногоШтрихкода.ИННВладельца;
			СтрокаШтрихкода.ЕдиницаТовара = НайденнаяСтрокаЗапрошенногоШтрихкода.ЕдиницаТовара;
			СтрокаШтрихкода.Статус = НайденнаяСтрокаЗапрошенногоШтрихкода.Статус;
			ЗапросСоставаКМСкопироватьСтрокиДерева(СтрокаШтрихкода, НайденнаяСтрокаЗапрошенногоШтрихкода);
		КонецЕсли;
	...
	ЗапросСоставаКМПреобразоватьДеревоВМассив(ДеревоШтрихкодов, СоставКодовМаркировки);
...
Функция ЗапросСоставаКМДеревоШтрихкодов()
	
	Дерево = Новый ДеревоЗначений();
	Дерево.Колонки.Добавить("Штрихкод", ОбщегоНазначения.ОписаниеТипаСтрока(200));
	Дерево.Колонки.Добавить("НормализованныйШтрихкод", ОбщегоНазначения.ОписаниеТипаСтрока(200));
	Дерево.Колонки.Добавить("Ошибка", ОбщегоНазначения.ОписаниеТипаСтрока(0));
	Дерево.Колонки.Добавить("ИННВладельца", ОбщегоНазначения.ОписаниеТипаСтрока(12));
	Дерево.Колонки.Добавить("ЕдиницаТовара", ОбщегоНазначения.ОписаниеТипаСтрока(150));
	Дерево.Колонки.Добавить("Статус", ОбщегоНазначения.ОписаниеТипаСтрока(150));

	Возврат Дерево;
	
КонецФункции

Функция ЗапросСоставаКМСформироватьДеревоЗапрашиваемыхШтрихкодов(Штрихкоды)
	
	Дерево = ЗапросСоставаКМДеревоШтрихкодов();
	...
	Если Штрихкоды.Количество() > 0 Тогда
		...
		Для индекс = 0 По Штрихкоды.ВГраница() Цикл
			...
			Если Дерево.Строки.Найти(Штрихкоды[индекс], "Штрихкод", Ложь) <> Неопределено Тогда
				Продолжить;
			КонецЕсли;
			
			СтрокаДереваРодитель = Дерево.Строки.Добавить();
			СтрокаДереваРодитель.Штрихкод = Штрихкоды[индекс];
			
			РезультатРазбора = РазобратьКодМаркировки(СтрокаДереваРодитель.Штрихкод, НастройкиРазбораКодаМаркировки, 
				НастройкиРазбораКодаМаркировки.ДоступныеВидыПродукции);
			
			Если Не ПустаяСтрока(РезультатРазбора.Ошибка) Тогда
				СтрокаДереваРодитель.Ошибка = РезультатРазбора.Ошибка;
				Продолжить;
			КонецЕсли;
			
			ПредпологаемыеШтрихкоды = Новый Массив();
			...
			Для Каждого ДанныеРезультатаРазбора Из РезультатРазбора.Данные Цикл
				...
				Для Каждого ВидПродукции Из ДанныеРезультатаРазбора.ВидыПродукции Цикл
					...
					Если ОбщегоНазначенияИСПовтИсп.ЭтоПродукцияМОТП(ВидПродукции) Тогда
						...
						ШтрихкодБезСкобки = РазборКодаМаркировкиИССлужебныйКлиентСервер.НормализоватьКодМаркировки(
							ДанныеРезультатаРазбора, ВидПродукции, ПараметрыНормализацииМОТП);
						
						Если ПредпологаемыеШтрихкоды.Найти(ШтрихкодБезСкобки) = Неопределено Тогда
							ПредпологаемыеШтрихкоды.Добавить(ШтрихкодБезСкобки);
						КонецЕсли;
			...
			Если ПредпологаемыеШтрихкоды.Количество() > 0 Тогда
				
				Для индекс2 = 0 По ПредпологаемыеШтрихкоды.ВГраница() Цикл
					
					ПодчиненнаяСтрока = СтрокаДереваРодитель.Строки.Добавить();
					ПодчиненнаяСтрока.Штрихкод = ПредпологаемыеШтрихкоды[индекс2];
					ПодчиненнаяСтрока.НормализованныйШтрихкод = ДанныеРезультатаРазбора.НормализованныйКодМаркировки;
			...
			Иначе
				...
				ПодчиненнаяСтрока = СтрокаДереваРодитель.Строки.Добавить();
				ПодчиненнаяСтрока.Штрихкод = СтрокаДереваРодитель.Штрихкод;
				ПодчиненнаяСтрока.НормализованныйШтрихкод = ДанныеРезультатаРазбора.НормализованныйКодМаркировки;
...
Функция ЗапросСоставаКМПолучитьШтрихкодыКЗапросу(ДеревоШтрихкодов, ИмяКолонки)
	
	ШтрихкодыКЗапросу = Новый Массив();
	
	Для Каждого СтрокаШтрихкода Из ДеревоШтрихкодов.Строки Цикл
		
		Для Каждого ПодчиненнаяСтрокаШтрихкода Из СтрокаШтрихкода.Строки Цикл
			
			Если ШтрихкодыКЗапросу.Найти(ПодчиненнаяСтрокаШтрихкода[ИмяКолонки]) = Неопределено Тогда
				ШтрихкодыКЗапросу.Добавить(ПодчиненнаяСтрокаШтрихкода[ИмяКолонки]);
			КонецЕсли;
	...
	Возврат ШтрихкодыКЗапросу;
	
КонецФункции

Функция ЗапросСоставаКМСформироватьДеревоРезультат()
	
	Дерево = Новый ДеревоЗначений();
	Дерево.Колонки.Добавить("ЗапрашиваемыйШтрихкод", ОбщегоНазначения.ОписаниеТипаСтрока(200));
	Дерево.Колонки.Добавить("Штрихкод", ОбщегоНазначения.ОписаниеТипаСтрока(200));
	Дерево.Колонки.Добавить("Ошибка", ОбщегоНазначения.ОписаниеТипаСтрока(0));
...
Функция ЗапросСоставаКМПолучитьШтрихкодыПоДаннымИБ(ДеревоШтрихкодов)
	
	Результат = ЗапросСоставаКМДеревоШтрихкодов();
	
	Штрихкоды = ЗапросСоставаКМПолучитьШтрихкодыКЗапросу(ДеревоШтрихкодов, "НормализованныйШтрихкод");
	...
	Если Штрихкоды.Количество() > 0 Тогда
		...
		Запрос.Текст =
			...
			|ГДЕ
			|	ШтрихкодыУпаковокТоваров.ЗначениеШтрихкода В (&ШтрихкодыКЗапросу)";
		
		Запрос.УстановитьПараметр("ШтрихкодыКЗапросу", Штрихкоды);
		
		УстановитьПривилегированныйРежим(Истина);
		
		ДеревоУпаковок = ШтрихкодированиеИС.ВложенныеШтрихкодыУпаковок(
			Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Ссылка")).ДеревоУпаковок;
		
		ЗапросСоставаКМСкопироватьСтрокиДереваПоДаннымИБ(Результат, ДеревоУпаковок);
...
Процедура ЗапросСоставаКМСкопироватьСтрокиДереваПоДаннымИБ(СтрокаПриемник, СтрокаИсточник)
	
	Для Каждого СтрокаИсточника Из СтрокаИсточник.Строки Цикл
		
		НоваяСтрока = СтрокаПриемник.Строки.Добавить();
		НоваяСтрока.Штрихкод = СтрокаИсточника.Штрихкод;
		НоваяСтрока.НормализованныйШтрихкод = СтрокаИсточника.Штрихкод;

		Если СтрокаИсточника.ВидУпаковки = Перечисления.ВидыУпаковокИС.Потребительская Тогда
			НоваяСтрока.ЕдиницаТовара = "SINGLE";
		Иначе
			НоваяСтрока.ЕдиницаТовара = "GROUP";
		КонецЕсли;

		ЗапросСоставаКМСкопироватьСтрокиДереваПоДаннымИБ(НоваяСтрока, СтрокаИсточника);
		
	КонецЦикла;
	
КонецПроцедуры

Процедура ЗапросСоставаКМСкопироватьСтрокиДерева(СтрокаПриемник, СтрокаИсточник)
	...
	Для Каждого СтрокаИсточника Из СтрокаИсточник.Строки Цикл
		...
		НоваяСтрока.Штрихкод = СтрокаИсточника.Штрихкод;
		НоваяСтрока.НормализованныйШтрихкод = СтрокаИсточника.Штрихкод;
		НоваяСтрока.ЕдиницаТовара = СтрокаИсточника.ЕдиницаТовара;
		НоваяСтрока.ИННВладельца = СтрокаИсточника.ИННВладельца;
		НоваяСтрока.Статус = СтрокаИсточника.Статус;

		ЗапросСоставаКМСкопироватьСтрокиДерева(НоваяСтрока, СтрокаИсточника);

	КонецЦикла;

КонецПроцедуры

Функция ЗапросСоставаКМНачатьПолучениеШтрихкодовПоДаннымСервиса(ДеревоШтрихкодов)
	
	ДеревоРезультат = ЗапросСоставаКМДеревоШтрихкодов();
	
	Штрихкоды = ЗапросСоставаКМПолучитьШтрихкодыКЗапросу(ДеревоШтрихкодов, "Штрихкод");
	
	Если Штрихкоды.Количество() > 0 Тогда
		
		КлючСессии = ПолучитьКлючСессии();
		
		ПорцииШтрихкодов = РазбитьМассивНаПорции(Штрихкоды, 1000);
		
		Для индекс = 0 По ПорцииШтрихкодов.ВГраница() Цикл
			
			ЗапросСоставаКМПолучитьШтрихкодыПоДаннымСервиса(КлючСессии, ПорцииШтрихкодов[индекс], ДеревоРезультат);
...
Процедура ЗапросСоставаКМПолучитьШтрихкодыПоДаннымСервиса(КлючСессии, ЗапрашиваемыеШтрихкоды, ДеревоРезультат)
	
	Если ЗапрашиваемыеШтрихкоды.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	РезультатОтправкиЗапроса = ЗапросСоставаКМВыполнитьЗапросВСервис(КлючСессии, ЗапрашиваемыеШтрихкоды);
	
	Для Каждого ЭлементРезультата Из РезультатОтправкиЗапроса.Объект Цикл

		КИЗ = СвойствоСтруктуры(ЭлементРезультата, "cisInfo.cis", "");
		КИЗЗапроса = СвойствоСтруктуры(ЭлементРезультата, "cisInfo.requestedCis", "");
		ВложенныеШК = СвойствоСтруктуры(ЭлементРезультата, "cisInfo.child", Новый Массив);
		Ошибка = СвойствоСтруктуры(ЭлементРезультата, "errorMessage", "");
		ИННВладельца = СвойствоСтруктуры(ЭлементРезультата, "cisInfo.ownerInn", "");
		ТипУпаковки = СвойствоСтруктуры(ЭлементРезультата, "cisInfo.packageType", "");
		Статус = СвойствоСтруктуры(ЭлементРезультата, "cisInfo.status", "");

		Если ПустаяСтрока(КИЗ) И Не ПустаяСтрока(КИЗЗапроса) Тогда
			КИЗ = КИЗЗапроса;
		ИначеЕсли Не ПустаяСтрока(КИЗ) И ПустаяСтрока(КИЗЗапроса) Тогда
			КИЗЗапроса = КИЗ;
		ИначеЕсли ПустаяСтрока(КИЗ) И ПустаяСтрока(КИЗЗапроса) Тогда
		...
		Если ПустаяСтрока(Ошибка) Тогда
			...
			Если ПустаяСтрока(КИЗЗапроса) Тогда
				ТекстОшибки = НСтр("ru = 'Не соответствие API ГИС МТ.
				|Обратитесь в техническую поддержку!
				|
				|%1'");
			...
			НоваяСтрока.Штрихкод = КИЗЗапроса;
			НоваяСтрока.НормализованныйШтрихкод = КИЗ;
			НоваяСтрока.ИННВладельца = ИННВладельца;
			НоваяСтрока.ЕдиницаТовара = ?(ТипУпаковки = "UNIT", "SINGLE", "GROUP");
			НоваяСтрока.Статус = Статус;

			ЗапросСоставаКМПолучитьШтрихкодыПоДаннымСервиса(КлючСессии, ВложенныеШК, НоваяСтрока);
...
Функция ЗапросСоставаКМВыполнитьЗапросВСервис(КлючСессии, Штрихкоды)
	...
	РезультатЗапроса = ОтправитьДанныеВСервис(URLЗапроса, Штрихкоды, КлючСессии, "POST", ПараметрыОтправки);
...
Процедура ЗапросСоставаКМПреобразоватьДеревоВМассив(СтрокаДерева, МассивРезультат)
	
	Для Каждого Строка Из СтрокаДерева.Строки Цикл
		
		Если ПустаяСтрока(Строка.Ошибка) Тогда

			ДанныеСтроки = Новый Структура;
			ДанныеСтроки.Вставить("requestedCis", Строка.Штрихкод);
			ДанныеСтроки.Вставить("cis", Строка.НормализованныйШтрихкод);
			ДанныеСтроки.Вставить("child", Новый Массив);
			ДанныеСтроки.Вставить("ownerInn", Строка.ИННВладельца);
			ДанныеСтроки.Вставить("packType", Строка.ЕдиницаТовара);
			ДанныеСтроки.Вставить("status", Строка.Статус);

			ЗапросСоставаКМПреобразоватьДеревоВМассив(Строка, ДанныеСтроки.child);
			
		Иначе
			
			ДанныеСтроки = Новый Структура();
			ДанныеСтроки.Вставить("requestedCis", 	Строка.Штрихкод);
			ДанныеСтроки.Вставить("error", 			Строка.Ошибка);
